...
% small functions.
function P = rosa_lineextract(P, RADIUS, bUpdateConnectivity,app)
...
if (bUpdateConnectivity)
    ...
    [P.spls, P.spls_adj,P.corresp] = edge_collapse_update(P.cpts, P.spls, P.corresp, P.rings, P.spls_adj);
else
    [P.spls,P.corresp] = farthest_sampling_by_sphere(P.cpts, P.sample_radius,app,P);
    if size(P.spls,1)==1
        P.sphere=true;
        return
    end
    P.spls_adj = connect_by_inherit_neigh(P.cpts, P.spls, P.corresp, P.rings,app);%Establishing the initial connectivity matrix
    [P.spls, P.spls_adj,P.corresp] = edge_collapse(P.cpts, P.spls, P.corresp, P.rings, P.spls_adj,app,P);
end

if (SHOW_RESULTS)
    figure(1); movegui('northeast');set(gcf,'color','white');hold on;
