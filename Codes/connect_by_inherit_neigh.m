...
% @data:   2010-5-7
% @version 2.0
function A = connect_by_inherit_neigh(pts, spls, corresp, neigh,app)
...
for pIdx=1:length(pts)    
    ...
    pc = corresp(pIdx);
    if pc == 0
        WriteSkeletonMessages('Warning: some points have no correspondence.',app)
        continue;
    end
...
for i=1:size(A,1)
    ...
    if length( find( A(i,:)>0) ) == 1
        isopts(1, end+1) = i;
        WriteSkeletonMessages(['Warning: there are isolate points: ' num2str(i) '.'],app)
    end
end
